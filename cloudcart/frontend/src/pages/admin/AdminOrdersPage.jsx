  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        // If no orders in DB, show mock data for demo
        setOrders(apiOrders.length > 0 ? apiOrders.map(o => ({
          _id: o._id,
        ...
        // Fallback to mock data if API fails (e.g. no auth token)
                ...
                <tbody>
                  {filtered.map((o, i) => (
                    <motion.tr key={o._id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                      ...
                      <td className="p-4 font-medium text-accent-primary">{o._id.slice(-8)}</td>
                      <td className="p-4">
